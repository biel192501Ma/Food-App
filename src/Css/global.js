...
font-weight:${({bold , regular}) => bold ? '900' :  regular ? '300': '500' } ;
...
width: 100% ;
height: 50px ;
background-color: ${({color}) => color ? theme.Colors[color] : theme.Colors.graydark} ;
border-radius: 5px ;
margin-top: ${({top}) => top ? top  + '%': '0' };
   margin-bottom: ${({bottom}) => bottom ? bottom + '%': '0' };
   margin-left: ${({left}) => left ? left+ '%' : '0' };
   padding: 10px ;
   color: ${({textcolor}) => textcolor ? theme.Colors[textcolor] : theme.Colors.dark};
